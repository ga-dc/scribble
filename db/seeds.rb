...
{:content=>"What's a knock-out like you doing in a computer-generated gin joint like this?", :username=>"Guinan", :post=>Post.find_by(title: "Encounter at Farpoint")},
...
{:content=>"How long can two people talk about nothing?", :username=>"Worf", :post=>Post.find_by(title: "Where No One Has Gone Before")}, {:content=>"Mr. Worf, you do remember how to fire phasers?", :username=>"Wesley Crusher", :post=>Post.find_by(title: "Where No One Has Gone Before")}, {:content=>"Some days you get the bear, and some days the bear gets you. Maybe if we felt any human loss as keenly as we feel one of those close to us, human history would be far less bloody.", :username=>"Troi's Mom", :post=>Post.find_by(title: "Where No One Has Gone Before")},

{:content=>"About four years. I got tired of hearing how young I looked.", :username=>"Worf", :post=>Post.find_by(title: "Shades of Grey")}, {:content=>"Could someone survive inside a transporter buffer for 75 years?", :username=>"Captain Sisko", :post=>Post.find_by(title: "Shades of Grey")}, {:content=>"I'll alert the crew. The Federation's gone; the Borg is everywhere!", :username=>"Quark", :post=>Post.find_by(title: "Shades of Grey")},

{:content=>"Damage report! Your head is not an artifact! Captain, why are we out here chasing comets?", :username=>"Doctor Crusher", :post=>Post.find_by(title: "True Q")}, {:content=>"What's a knock-out like you doing in a computer-generated gin joint like this?", :username=>"Wesley Crusher", :post=>Post.find_by(title: "True Q")}, {:content=>"You enjoyed that.", :username=>"Q", :post=>Post.find_by(title: "True Q")},

{:content=>"Wait a minute - you've been declared dead. You can't give orders around here.", :username=>"Worf", :post=>Post.find_by(title: "All Good Things...")}, {:content=>"I've had twelve years to think about it. And if I had it to do over again, I would have grabbed the phaser and pointed it at you instead of them.", :username=>"Wesley Crusher", :post=>Post.find_by(title: "All Good Things...")}, {:content=>"The unexpected is our normal routine!", :username=>"Geordi", :post=>Post.find_by(title: "All Good Things...")}
])
